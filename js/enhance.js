...
(function () {
  ...
  // 失败时 reject(Error)，把 python 报的真实原因带出去（不再静默返回空串）
  ...
  async function runAll() {
    ...
    try {
      ...
      for (var i = 0; i < totalN; i++) {
        ...
        try { await exportOne(seqName, presetPath, outFile, rangeArg); } catch (e) {
    ...
    try {
      ...
      for (var k = 0; k < jobs.length; k++) {
        ...
        try {
          ...
          failN++;
          log('✗ 下载失败（任务 ' + job.id + '）：' + ((e && e.message) || e), 'err');
        }
      }
    ...
    (async function () {
      try {
        ...
        if (dlFile && fs.existsSync(dlFile)) {
          ...
          log('⚠ 下载失败（任务 ' + taskId + '），原因未知', 'err');
          if (progEl) progEl.textContent = '下载失败';
        }
      } catch (e) {
        log('✗ 下载失败（任务 ' + taskId + '）：' + ((e && e.message) || e), 'err');
        if (progEl) progEl.textContent = '下载失败';
      } finally {
  ...
  function refreshAccount() {
    ...
    cp.exec('"' + py + '" "' + script + '" account', { windowsHide: true, timeout: 20000 }, function (err, stdout) {
      ...
      if (j && j.configured) {
        ...
        enAccHint.textContent = '⚠ 未配置超分站账号，请点「⚙ 账号」设置（否则会用默认账号）';
