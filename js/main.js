// vh-Atelier 整合插件（lite 分发版）- 两级导航切换逻辑
// 工作台（组）→ 子功能（面板）。组：字幕、声音、交付、剧本
(function () {
    var panels = {
    ...
    // 默认显示组（lite 版没有进度组，默认进字幕）
    var DEFAULT_GROUP = 'sub';
    // tab 归属映射
    var groupOf = {};
    ...
    window.__atHideScriptGroup = function () {
        if (currentInGroup['script'] && groupBtns.length) {
            ...
            if (active === 'script') switchGroup(DEFAULT_GROUP);
        }
