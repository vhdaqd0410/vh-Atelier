// vh-Atelier 板块：剧本阅读（lite 独立版，不依赖视频工作台）
// 剧本库：手动打开 docx/pdf + 最近阅读 + 已固定 + 上次阅读位置记忆
(function () {
    var fs = require('fs');
    ...
    function loadScriptDocx(docx, projectName, restoreEp) {
        try { console.log('[vh-script] 解析剧本: ' + path.basename(docx)); } catch (e) {}
        var py = findPython();
    ...
    function showScriptHome() {
        ...
        sub.textContent = '直接打开本地剧本（docx / pdf）阅读，支持续读与翻译';
        box.appendChild(sub);
        ...
        if (recents.length === 0 && fixed.length === 0) {
            ...
            tip.innerHTML = '还没有剧本记录。<br>点上方「📂 打开剧本文件…」选一个 docx / pdf 开始阅读。<br><br>打开过的会自动出现在「最近阅读」，方便下次直接点。';
            box.appendChild(tip);
        }
