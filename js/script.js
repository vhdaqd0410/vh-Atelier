 ...
 * 外部依赖：window.__vhProgress.setStatus(text)  写状态栏文字（可选，缺失时退回控制台）
 */
(function () {
    var fs = require('fs');
    ...
    // 写状态栏（原由 progress.js 提供；插件A已移除进度板块，故退回控制台）
    function setStatus(msg) {
        try {
            var p = window.__vhProgress;
            if (p && p.setStatus) { p.setStatus(msg); return; }
            if (msg) console.log('[script] ' + msg);
        } catch (e) {}
    }
    ...
    // 复制到剪贴板（原由 progress.js 提供，插件A已移除进度板块，这里恢复）
